winnerGenderMap<-function(input, output, session, parentsession,dname,conmanager){
...
plotMap<-function( year, options, plot, envr){
  ...
  title<-paste0("Gender wise winners for loksabha in ",yr)
      base<-base %>% 
...
parentsession$output$ge_filter_selection<-renderUI({
 ...
 checkboxGroupInput(ns("filter_pname") , "Select gender type ", c())
 } 
 else {
shinyjs::hidden(checkboxGroupInput(ns("filter_pname") , "Select gender type ", c())) 
 }
 tagList (
 tmp1,
 tmp2) 
