...
#        browser()
      ...
      print(paste('Voteshare-stchange: statename is ',st))
        b<-read.csv(paste0(dirname,"VoteShares.csv"))%>%filter(state==st)
  ...
  ShowAll<-function(){
    ...
    parentsession$output$ae_filter_selection<-renderUI({
      ...
      b<-read.csv(paste0(dirname,"VoteShares.csv"))%>%filter(state==sname)
    ...
    parentsession$output$distPlot <- renderPlotly({
      ...
      lapply(selectedpartynames,function(x) {
        ...
        base<<-add_trace(base,y=~get(x),name=x,type='scatter',mode='lines+markers')
        }
        )
      ...
      base %>%
      layout(title = paste0("Party wise vote shares across years in ",sname),
             xaxis = list(title = "Year"),
             yaxis = list (title = "Vote share in percentage"))
    })
