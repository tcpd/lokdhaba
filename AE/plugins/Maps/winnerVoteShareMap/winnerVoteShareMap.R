...
#################################Fixed:################################################################
########################################################################################################
winnerVoteShareMap <- function(input, output, session, parentsession,statename_reactive,dirname) {
  ...
  obs_sname<-observe({
    ...
    if(!is.null(st) && trimws(st)!=""){
      print('statename observer')
        ...
        print(paste('Winner voteshare map-stchange: statename is ',st))
  ...
  HideAll<-function(){
    ...
    print('Winner VoteShare map: Hidden all')
  }
  ShowAll<-function(){
    ##show all components 
...
current_filters$party<<-party
        ...
        winners$vote_percent[winners$party1!=party]<-NA
    ...
    parentsession$output$mapPlot <- renderLeaflet({
      ...
      if(length(selectedfilters)==0){
        print('winner vote share map: returning')
        return()
      }
