...
#################################Fixed:################################################################
########################################################################################################
winnerMarginMap <- function(input, output, session, parentsession,statename_reactive,dirname) {
  ...
  HideAll<-function(){
    ...
    print('Winner margin map: Hidden all')
  }
  ShowAll<-function(){
    ##show all components 
    ...
    parentsession$output$mapPlot <- renderLeaflet({
      ...
      if(length(selectedpercentage)==0){
        print('winnermap margin: returning')
        return()
      }
