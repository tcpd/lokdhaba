...
#################################Fixed:################################################################
########################################################################################################
numCandidatesMap <- function(input, output, session, parentsession,statename_reactive,dirname) {
  ...
  values=reactiveValues(snameset=1,yearselected="",count=c())
  ...
  if("ASSEMBLY" %in% names(shape)){
    ...
    parentsession$output$mapPlot <- renderLeaflet({
      ...
      if(length(selectedcount)==0){
        print('num candidates map : returning')
        return()
      }
