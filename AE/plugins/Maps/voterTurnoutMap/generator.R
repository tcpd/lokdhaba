...
filters = "filterid,construction,isinit
           1,selectInput(ns(\"voterTI_year\"):\"Select Year\": c() : selectize = TRUE),T
           2,checkboxGroupInput(ns(\"voterT_names\") : \"Select range \": c()),F"
    
inputtable="filterid,name,type,alias
            1,statename_reactive(),string,selected_stname 
            2,input$voterTI_year,string,selected_year
            3,input$voterTI_year,string,selected_year
            3,input$voterT_names,list,selected_range"

##default can be allmultiple/allsingle/emptymultiple/emptysingle/NA.
outputtable="filterid,name,type,alias,default,label 
            1,voterTI_year, updateSelectInput,yearlist,emptysingle,\"Select Year\"
            2,voterT_names, updateCheckboxGroupInput,options,allmultiple,\"Select range\"
