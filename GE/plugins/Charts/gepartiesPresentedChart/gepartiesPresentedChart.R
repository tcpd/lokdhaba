...
gepartiesPresentedChart <- function(input, output, session, parentsession,dname) {
  ...
  HideAll<-function(){
    ...
    print('ge parties contested represented: Hidden all')
  }
  ShowAll<-function(){
    ##show all components 
    ...
    parentsession$output$distPlot <- renderPlotly({
      ...
      if(length(selectedoptionnames)==0){
        print('ge Parties contested represented chart: returning')
        return()
      }
    ...
    print('ge Parties contested represented chart : Enabled all')
  }
