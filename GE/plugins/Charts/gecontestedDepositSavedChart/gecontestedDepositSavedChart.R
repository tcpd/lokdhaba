...
#################################Fixed:################################################################
########################################################################################################
gecontestedDepositSavedChart <- function(input, output, session, parentsession,dname) {
  ...
  HideAll<-function(){
    ...
    print('ge contested deposit saved: Hidden all')
  }
  ShowAll<-function(){
    ##show all components 
    ...
    parentsession$output$ge_filter_selection<-renderUI({
      ...
      # print(paste("ge contested deposit saved -fill filter: statename is ",sname))
    ...
    parentsession$output$distPlot <- renderPlotly({
      ...
      if(length(selectedoptionnames)==0){
        print('ge contested deposit lost chart: returning')
        return()
      }
    ...
    print('ge contested and deposit saved chart : Enabled all')
  }
