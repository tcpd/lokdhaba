...
#################################Fixed:################################################################
########################################################################################################
partiesPresentedChart <- function(input, output, session, parentsession,statename_reactive,dname) {
  ...
  obs_sname<-observe({
    ...
    if(!is.null(st) && trimws(st)!=""){
      st<-gsub(" ","_",st)
      print(dirname)
      print(paste('Parties contested represented-stchange: statename is ',st))
        b<-readPartiesContestedRepresentedFile(st)
  ...
  ShowAll<-function(){
    ...
    parentsession$output$distPlot <- renderPlotly({
      ...
      if(length(selectedoptionnames)==0){
        print('Parties contested represented chart: returning')
        return()
      }
    ...
    print('Parties contested represented chart : Enabled all')
  }
